...
public class CarManager implements CarService {
	...
	public void update(UpdateReservationRequest updateReservationRequest) {
	    // Önce mevcut car'ı bul
	    Car existingCar = carRepository.findById(updateReservationRequest.getId())
	        .orElseThrow(() -> new CarNotFoundException(
	            updateReservationRequest.getId() + " numaralı araç bulunamadı."));
	    
	    // Direkt mapping yap (id dahil)
	    Car updatedCar = this.modelMapperService.forRequest()
	        .map(updateReservationRequest, Car.class);
